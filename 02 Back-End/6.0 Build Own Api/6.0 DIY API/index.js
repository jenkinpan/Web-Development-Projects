...
app.get("/jokes/:id", (req, res) => {
  ...
  const specJoke = jokes.find((joke) => joke.id === parseInt(req.params.id));
  // ! loop through jokes array to find joke with id that matches req.params.id
  res.json(specJoke);
});
...
app.delete("/jokes/:id", (req, res) => {
  ...
  if (jokeIndex > -1) {
    ...
    res.status(200).json(`Joke with id ${id} has been deleted.`);
  } else {
    res.status(404).json({
      error: `Joke with id ${id} does not exist. No joke was deleted.`,
    });
  }
});
